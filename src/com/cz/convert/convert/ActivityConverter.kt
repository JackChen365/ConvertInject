 ...
 * activity转换器对象
 */
class ActivityConverter(factory: PsiElementFactory, file: PsiJavaFileImpl, clazz: PsiClass, configuration: Configuration, single: Boolean) : BaseConverter(factory, file, clazz, configuration, single) {
                    ...
                    if(null!=firstChild){
                        ...
                        if (null != innerChild) {
                            if(null!=lastChild && innerChild.text == "setContentView"){
                                //加入方法,必须为body体添加元素,否则报断言失败 Assertion failed: anchorBefore == null || anchorBefore.getTreeParent() == parent
